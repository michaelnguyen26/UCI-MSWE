    ...
    public static void transferMoney(final Account fromAccount,
		...
		class Helper { 
			public void transfer() throws InsufficientFundsException { 
				// synchronized (fromAccount) {  //<-- nested lock acquisition usually leads to deadlocks
		...
		if (fromHash < toHash) {   //ordered synchronization by syncing the smallest first
			...
			synchronized (tieLock) { //same amount, tielock to sync both 
				synchronized (fromAccount) { 
					synchronized (toAccount) { 
						new Helper().transfer(); 
					} 
