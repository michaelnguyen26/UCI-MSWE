public class JDisplay2 implements HighLevelDisplay {
    ...
    public synchronized void addRow(String str) { //synchornized method to allow another method call to wait if not finished
		updateRow(usedRows,str);
		flashRow(usedRows,1000);
		usedRows++;
    }

    public synchronized void deleteRow(int row) { //synchornized method to allow another method call to wait if not finished
	if (row < usedRows) {
